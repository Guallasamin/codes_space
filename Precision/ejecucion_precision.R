...
tbl_ejec <-
  dbGetQuery(conexion,'SELECT * FROM st04_tablas_ejecucion WHERE precision=\'TRUE\'')

if(nrow(tbl_ejec) > 1) {
  stop("El proceso se detuvo porque hay más de un registro en la tabla.")
} else {
  tabla <- tbl_ejec$tabla_escritura[1]  # Asegurándonos de que estamos seleccionando una fila específica
  ...
  tabla_prec <- dbGetQuery(conexion, 
                           glue("SELECT 
                               ...
                               WHERE tabla_entrada = '{tabla}'"))
  
  query_pob <- ifelse(tabla == 'rc02_revit_nor',
                      "SELECT * FROM st05_registro_poblacion WHERE substring(id_hash_rp,1,5)=\\\'00101\\\'",
                      "SELECT * FROM st05_registro_poblacion")
